...
public class AnchorTag extends AbstractClosingTag {
	...
	public int doEndTag() throws JspException {
			// 当前登陆用户
			SpEmployeeBinfo currentEmpl = (SpEmployeeBinfo) pageContext.getSession().getAttribute("spEmployeeBinfo");
			// 当前准备显示的链接对应权限Url
			// >在开头加下‘/’
			String emplprivUrl = action;
			// 是否有权限的
			if (currentEmpl.hasPrivilegeByUrl(emplprivUrl)) {
				return super.doEndTag();//正常的声称并显示超链接标签，并继续执行页面中后面的代码
			} else {
				return EVAL_PAGE;//不声称与显示超链接标签,什么都不做，只是继续执行页面的代码
			}
	}
